...
do
    ...
    ssh $host '
        # Kill all processes matching the pattern /tmp/go-build*/b001/exe/node_manager
        pkill -f "/tmp/go-build.*\/b001\/exe\/node_manager"

        # Check if the process was killed
        if ps -p $(pgrep -f "/tmp/go-build.*\/b001\/exe\/node_manager") > /dev/null; then
