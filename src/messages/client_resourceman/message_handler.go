...
func (m *MessageHandler) ClientRequestSend(wrapper *ClientJobRequest) error {
...
func (m *MessageHandler) ResourceManagerResponseSend(wrapper *ResourceManagerMessage) error {
	serialized, err := proto.Marshal(wrapper)
	if err != nil {
		return err
	}
...
func (m *MessageHandler) ClientRequestReceive() (*ClientJobRequest, error) {
	prefix := make([]byte, 8)
	m.readN(prefix)
	...
	wrapper := &ClientJobRequest{}
	err := proto.Unmarshal(payload, wrapper)
	return wrapper, err
}

func (m *MessageHandler) ResourceManagerResponseReceive() (*ResourceManagerMessage, error) {
	prefix := make([]byte, 8)
	m.readN(prefix)
	...
	wrapper := &ResourceManagerMessage{}
	err := proto.Unmarshal(payload, wrapper)
	if err != nil {
		fmt.Printf("err: %s\n", err)
	}
