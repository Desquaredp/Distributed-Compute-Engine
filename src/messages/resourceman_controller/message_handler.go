...
func (m *MessageHandler) ClientRequestSend(wrapper *ResourceManagerRequest) error {
...
func (m *MessageHandler) ServerResponseSend(wrapper *ControllerResponse) error {
	serialized, err := proto.Marshal(wrapper)
	if err != nil {
		return err
	}
...
func (m *MessageHandler) ClientRequestReceive() (*ResourceManagerRequest, error) {
	prefix := make([]byte, 8)
	m.readN(prefix)
	...
	wrapper := &ResourceManagerRequest{}
	err := proto.Unmarshal(payload, wrapper)
	return wrapper, err
}

func (m *MessageHandler) ServerResponseReceive() (*ControllerResponse, error) {
	prefix := make([]byte, 8)
	m.readN(prefix)
